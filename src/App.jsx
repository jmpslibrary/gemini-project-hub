...
import { 
  ...
  ArrowLeft, Trash2, Search, Lock, User, LogOut, Globe
} from 'lucide-react';
...
// --- Component: Project Viewer (The "Browser") ---
...
const UploadForm = ({ onCancel, onSubmit }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto py-12 px-4">
      <button onClick={onCancel} className="text-slate-500 hover:text-slate-800 flex items-center gap-2 mb-8 transition-colors">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50">
          ...
          <p className="text-slate-500 text-sm mt-1">Share a web app, tool, or experiment.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div>
            ...
            <input 
              ...
              className="w-full border-slate-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 px-4 py-2.5 border outline-none transition-all"
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full border-slate-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 px-4 py-2.5 border outline-none transition-all h-24 resize-none"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2 flex justify-between">
              ...
              <span className="text-xs text-slate-400">HTML/CSS/JS or React Component</span>
            </label>
            <div className="relative">
              <textarea 
                required value={code} onChange={e => setCode(e.target.value)} 
                className="w-full bg-slate-900 text-slate-300 font-mono text-xs rounded-lg shadow-inner p-4 border border-slate-800 outline-none focus:ring-2 focus:ring-indigo-500 h-64"
                placeholder="Paste your Gemini code here..."
                spellCheck="false"
              />
            </div>
          </div>
          ...
          <div className="pt-4 flex justify-end gap-4">
            ...
            <button 
              type="submit" 
              disabled={loading}
              className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-medium shadow-sm shadow-indigo-200 transition-all flex items-center gap-2"
            >
      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-4">
          <div className="bg-white p-8 rounded-2xl shadow-lg border border-slate-100 max-w-md w-full text-center">
            ...
            <p className="text-slate-500 mb-8">You must be logged in to contribute new projects to the hub.</p>
            <button onClick={toggleLogin} className="w-full bg-indigo-600 text-white py-3 rounded-lg font-semibold hover:bg-indigo-700 transition-colors">
              Log In with Google
            </button>
            <button onClick={() => navigate('#/')} className="mt-4 text-slate-400 hover:text-slate-600 text-sm">
              Cancel
            </button>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:flex relative group">
              ...
              <input 
                ...
                className="pl-9 pr-4 py-2 bg-slate-100 border-transparent rounded-lg text-sm focus:bg-white focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all w-64 outline-none"
              />
            </div>
            ...
            <button 
              onClick={toggleLogin}
              className={`text-sm font-medium px-3 py-2 rounded-lg transition-colors flex items-center gap-2 ${isCreator ? 'text-indigo-600 bg-indigo-50' : 'text-slate-500 hover:bg-slate-100'}`}
            >
            ...
            <button 
              ...
              className="bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-all shadow-md shadow-slate-200"
            >
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        <div className="mb-10">
          ...
          <p className="text-slate-500 max-w-2xl">
            A collection of web applications, tools, and experiments created with Gemini. 
            Run them directly in your browser or view the source code.
          </p>
        </div>
        ...
        {projects.length === 0 ? (
          <div className="border-2 border-dashed border-slate-200 rounded-2xl p-12 text-center bg-slate-50/50">
            ...
            <p className="text-slate-500 mb-6">Get started by uploading the first project.</p>
            <button onClick={() => navigate('#/upload')} className="text-indigo-600 font-medium hover:underline">
              Upload Project
            </button>
          </div>
        ) : (
            ...
            {filteredProjects.map(p => (
              ...
              >
                ...
                <div className="px-6 py-4 border-t border-slate-50 bg-slate-50/50 group-hover:bg-white transition-colors flex items-center justify-between">
                  ...
                  <span className="text-xs font-medium text-indigo-600 opacity-0 group-hover:opacity-100 transition-all flex items-center gap-1">
                    Launch <ArrowLeft className="w-3 h-3 rotate-180" />
                  </span>
                </div>
              </div>
            ))}
